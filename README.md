...
Recall 将用户显式触发的工作节点转成可搜索、可学习的本地记忆。除了时间线，它会把零散记录归并为工作 Episode、持续维护项目状态、提出待确认的用户记忆与工作习惯，并在每日简报中优先展示真实进展、未闭环事项、少量高价值洞察和下一步行动。它的目标不是记录一切，而是在用户可见、可纠正、可删除的前提下逐渐理解什么对用户真正重要。
...
| 提醒 | 从待办/承诺类文本提出提醒候选；创建前由用户确认并选择日期时间，已安排提醒可随时取消。 |
...
项目提供无网络、无真实截图、无真实云端密钥的集成验证器。它覆盖事件规则、隐私脱敏、应用排除、模拟截图到 OCR 的记录管线、重复记录拦截、检索排序、提醒去重、通知宿主保护、本地持久化、自定义模型连接配置、长对话压缩、可追溯回答、Episode 聚合、重要记录选择、反馈抑制、习惯学习、SQLite FTS5 与旧状态迁移。另提供可选的真实 Anthropic 兼容服务探测。
...
```text
PASS: RecallVerifier completed 21 integration checks.
```
