...
| 记忆治理 | 支持时间线单条/批量删除、删除影响预览、总结条目级管理、彻底忘记来源、7 天最近删除与恢复，以及按删除原因学习的本地忽略规则。 |
...
Recall 将“删除一份总结”和“彻底忘记证据”区分为两个明确动作。前者只移除生成结果，保留原始记录以便重新总结；后者会让原始记录立即退出时间线、FTS 搜索、问一问上下文、每日总结、长期记忆和派生提醒，同时使相关日的智能状态在撤销窗口后基于剩余证据重新计算。由用户手动创建且没有来源关联的提醒不会被误删。

- **删除前可预见**：时间线支持管理模式、当天全选和批量删除；确认页会列出记录、截图、受影响总结、派生提醒、历史回答与智能状态数量。
- **删除后可恢复**：普通删除会进入本机“最近删除”并保留 7 天，界面即时提供撤销；可在“隐私与模型 → 记忆治理”中恢复或提前永久删除。
- **总结可逐项管理**：新旧每日总结都会转换为结构化条目，可只移除某个结论，也可沿其证据链彻底忘记关联记录。Markdown 仅作为兼容和导出格式保留。
- **聊天与派生状态同步清理**：引用被删除证据的助手回答会移出会话，长对话压缩摘要会失效；相关 Episode、项目状态、洞察、用户记忆、习惯和提醒随后从剩余证据重建。
- **从删除中学习**：删除时可选择“重复 / 不准确 / 不重要 / 敏感 / 不再需要”等原因，并可明确开启“以后也忽略相似内容”。匹配规则只保存在本机、可随时删除，默认不会因为一次删除自动扩大过滤范围。
