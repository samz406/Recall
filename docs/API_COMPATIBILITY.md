...
Recall 将模型类型与底层协议分开，用户可编辑服务地址、模型名称和 API Key；根据项目所有者要求，密钥作为普通文本仅保存在用户本机 Recall 配置中，不使用 Keychain，因此不会触发系统钥匙串授权弹窗。当前支持两种协议，均默认关闭网络外发，只有用户显式启用云端上下文发送后才会发起请求。
...
| 兼容 Anthropic API | `POST {baseURL}/v1/messages` | 标准 `X-Api-Key: {key}`、`anthropic-version`；MiniMax 域名同时发送 `Authorization: Bearer {key}` 回退 | `model`、`system`、`messages`、`max_tokens`、`temperature` | `content` 数组中的 `type: text` 块 |
